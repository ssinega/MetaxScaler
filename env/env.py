...
class SupportEnv:
    ...
    def close(self) -> None:
        """Graceful teardown. Marks episode done and clears ticket reference."""
        self._done = True
        self._ticket = None
        self._conversation_history = []
    ...
    def render(self) -> str:
        if self._ticket is None:
            return "[SupportEnv] No active ticket."
        return (
            f"[SupportEnv] Ticket #{self._ticket['id']} | "
            f"Step {self._step_count}/{MAX_STEPS} | "
            f"Task: {self._task_id} | "
            f"Done: {self._done}\n"
            f"Query: {self._ticket['query']}"
        )
    ...
    def _build_observation(self) -> Observation:
        ...
        return Observation(
            ...
            conversation_history=list(self._conversation_history),
            available_categories=["billing", "account", "technical"],
            available_priorities=["low", "medium", "high"],
            available_actions=["refund", "escalate", "guide"],
        )
